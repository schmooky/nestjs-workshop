...
// Symbol mapping for Firejoker symbols
...
const freeSpinsEl = document.getElementById('free-spins') as HTMLElement;
...
function connectToServer() {
  ...
  // Create new WebSocket connection - note the path change from burst to firejoker
  const wsUrl = `ws://localhost:3000/firejoker/slot?sessionId=${sessionId}`;
...
function handleSessionResponse(session: SessionResult) {
  ...
  // Update free spins display if available
  if (session.round.totalFreespins > 0) {
    freeSpinsEl.textContent = `Free Spins: ${session.round.totalFreespins}`;
  ...
  // Only enable bet select if not in free spins mode
  betSelect.disabled = session.round.currentGameMode === GameMode.FreeSpins;
  ...
  betSelect.value = session.round.bet.toString();
...
function handleSpinResponse(round: Round) {
  ...
  if (round.newFreespins > 0) {
    log(`Won ${round.newFreespins} free spins!`, 'success');
  }
  
  // Check for game mode change
  if (round.nextGameMode === GameMode.FreeSpins) {
    log(`Entering Free Spins mode!`, 'success');
    betSelect.disabled = true;
  } else if (currentRound?.currentGameMode === GameMode.FreeSpins && round.nextGameMode === GameMode.Regular) {
    log(`Free Spins completed!`, 'success');
  ...
  if (round.totalFreespins > 0) {
    freeSpinsEl.textContent = `Free Spins: ${round.totalFreespins}`;
    freeSpinsEl.style.display = 'block';
  } else {
    freeSpinsEl.style.display = 'none';
  }
...
function updateReels(frame: number[][], paylines: any[] = []) {
  ...
  for (let reelIndex = 0; reelIndex < frame.length; reelIndex++) {
    ...
    for (let rowIndex = 0; rowIndex < reelSymbols.length; rowIndex++) {
      ...
      // Add special classes for Firejoker symbols
      if (symbolValue === 9) { // Joker wild
        symbol.classList.add('wild');
      } else if (symbolValue === 8) { // Bonus scatter
        symbol.classList.add('scatter');
      } else if (symbolValue === 1) { // Seven (highest paying)
        symbol.classList.add('seven');
      }
...
// Initialize
log('Firejoker Client initialized');